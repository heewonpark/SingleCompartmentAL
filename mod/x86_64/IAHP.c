...
#define cai_init cai_init_IAHP
 double cai_init = 0.00024;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 6);
 	...
 	ivoc_help("help ?1 IAHP /home/park/github/SingleCompartmentAL/mod/x86_64/IAHP.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   Dcai = channel_flow - ctau * cai ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 channel_flow = - 0.002 * ica ;
 Dcai = Dcai  / (1. - dt*( ( - (ctau)*(1.0) ) )) ;
 return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   channel_flow = - 0.002 * ica ;
    cai = cai + (1. - exp(dt*(( - (ctau)*(1.0) ))))*(- ( channel_flow ) / ( ( - (ctau)*(1.0)) ) - cai) ;
   }
  return 0;
}
