...
static void nrn_alloc(Prop* _prop) {
  ...
  if (!nrn_point_prop_) {
 	...
 	_ppvar[3]._pval = &nrn_alloc_node_->_area; /* diam */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 GABAa /home/park/Programming/hoc/Belmabrouk/mod/x86_64/GABAa.mod\n");
...
/*CVODE*/
 static int _ode_spec1 () {_reset=0;
 {
   ...
   Rinf = T * Alpha / ( T * Alpha + 1.0 / GABAtau ) ;
   Rtau = 1.0 / ( ( Alpha * T ) + 1.0 / GABAtau ) ;
   DR = ( Rinf - R ) / Rtau ;
   }
 return _reset;
}
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 131 in file GABAa.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
