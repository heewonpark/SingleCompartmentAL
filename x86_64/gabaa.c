...
#define Erev Erev_GABA
 double Erev = -80;
#define Prethresh Prethresh_GABA
 double Prethresh = 0;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 3);
 	...
 	ivoc_help("help ?1 GABA /home/park/github/SingleCompartmentAL/x86_64/gabaa.mod\n");
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  release();
 if(error){fprintf(stderr,"at line 122 in file gabaa.mod:\n	SOLVE release\n"); nrn_complain(_p); abort_run(error);}
