...
#define stim1 stim1_intermitStimNoise
 extern double stim1( double );
 /* declare global and static user variables */
#define amp amp_intermitStimNoise
 double amp = 0;
 ...
 "amp_intermitStimNoise", &amp_intermitStimNoise,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 void _intermittent_stim_withNoise_reg() {
	int _vectorized = 0;
  _initlists();
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 3);
 	...
 	ivoc_help("help ?1 intermitStimNoise /home/park/github/SingleCompartmentAL/x86_64/intermittent_stim_withNoise.mod\n");
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   ...
   if ( t < del + t0 + interval * j  && t >= del + interval * j ) {
     ...
     i = i1 * ( 1.0 + 0.001 * normrand ( 0.0 , 1.0 ) ) ;
     cnt = 0.0 ;
     k = j ;
     }
